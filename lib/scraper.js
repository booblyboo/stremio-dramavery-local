...
async function getContentMatches(url, regex, headers = {}) {
  return new Promise((resolve, reject) => {
    let body = '';
    needle.get(url, { open_timeout: 3000, headers: headers })
...
async function _scrape(mirror) {
  ...
  } else if (mirror.url.match(/kshows\.to\/load\.php/i)) {
    const parsedUrl = pUrl.parse(mirror.url)
    return getContentMatches(mirror.url, /sources:\[{file: \'([^']+)\'/)
      .then(matches => {
        if (matches.length > 1)
          return {
            name: mirror.name,
            url: proxy.addProxy(matches[1], { headers: { referer: mirror.url, origin: parsedUrl.protocol + '//' + parsedUrl.host } })
          }
        else
          return false
      })
